...
\IEEEauthorblockA{ASU ID: 1225177265\\
Arizona State University, Tempe, AZ\\
\href{mailto:ltnguy58@asu.edu}{ltnguy58@asu.edu}}
\and
\IEEEauthorblockN{Dhanush Mugajji Shambulingappa}
\IEEEauthorblockA{ASU ID: 1237526292\\
Arizona State University, Tempe, AZ\\
\href{mailto:dmugajji@asu.edu}{dmugajji@asu.edu}}
\and
\IEEEauthorblockN{Chandra Shekhar Pavuluri}
\IEEEauthorblockA{ASU ID: 1236078196\\
Arizona State University, Tempe, AZ\\
\href{mailto:cpavulur@asu.edu}{cpavulur@asu.edu}}
\and
\IEEEauthorblockN{Adrian Zhang}
\IEEEauthorblockA{ASU ID: 1224664415\\
Arizona State University, Tempe, AZ\\
\href{mailto:awzhang1@asu.edu}{awzhang1@asu.edu}}
\and
\IEEEauthorblockN{Shashikant Nanda}
\IEEEauthorblockA{ASU ID: 1235508926\\
Arizona State University, Tempe, AZ\\
\href{mailto:snanda5@asu.edu}{snanda5@asu.edu}}
\and
\IEEEauthorblockN{Chitwandeep Kaur Palne}
\IEEEauthorblockA{ASU ID: 1231519031\\
Arizona State University, Tempe, AZ\\
\href{mailto:cpalne@asu.edu}{cpalne@asu.edu}}
}
...
\begin{IEEEkeywords}
fraud detection, graph neural networks, GraphSAGE, Neo4j, class imbalance handling, graph features, unsupervised clustering
\end{IEEEkeywords}
...
The runtime entry point is \texttt{CODE/main.py}. Fig.~\ref{fig:pipeline} shows Steps~1--8 in the exact order the script runs them (including E4 before E3); plots and metrics land under \texttt{EVALUATIONS/}. Per \texttt{README.md}, run commands from the repository root (for example \texttt{python CODE/main.py}). Long hyperparameter searches may require adjusting parallel backend settings on some operating systems.

\begin{figure}[!htbp]
\centering
\includegraphics[width=1.05\linewidth]{system_pipeline.png}
\caption{Linear view of \texttt{CODE/main.py}: Steps~1--8 run strictly
top to bottom. E1/E2 reuse stratified splits after random oversampling;
Step~6 (E4) executes before Step~7 (E3). Artifacts land under
\texttt{EVALUATIONS/}; the UI lives under \texttt{CODE/dashboard/}.}
\label{fig:pipeline}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\columnwidth]{confusion_matrices.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\columnwidth]{baseline_vs_hybrid.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1\columnwidth]{pr_curves_comparison.png}
...
Table~\ref{tab:full} and Fig.~\ref{fig:full_comparison} consolidate results across all ten models spanning E1 through E5. RF with graph features achieves the highest overall PR-AUC of 1.0000, while GraphSAGE embeddings (E3) come close at 0.9997 without any manual feature engineering. SVM is the clear negative outlier and standalone GraphSAGE (E4) trades precision for high recall --- a useful property in a recall-oriented triage stage.

\begin{table}[H]
\centering
\caption{Full Model Comparison Across All Experiments}
\label{tab:full}
\scriptsize
...
\textbf{Model} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} & \textbf{PR-AUC} \\
\midrule
LR (E1)              & 0.9491 & 0.8664 & 0.9059 & 0.9714 \\
SVM (E1)             & 0.2617 & 0.3141 & 0.2855 & 0.4904 \\
RF (E1)              & 0.9949 & 1.0000 & 0.9975 & 0.9990 \\
XGB (E1)             & 0.9432 & 1.0000 & 0.9708 & 0.9953 \\
LR + Graph (E2)      & 0.9682 & 0.9740 & 0.9711 & 0.9959 \\
\textbf{RF + Graph (E2)} & \textbf{0.9976} & \textbf{1.0000} & \textbf{0.9988} & \textbf{1.0000} \\
XGB + Graph (E2)     & 0.9792 & 1.0000 & 0.9895 & 0.9993 \\
GraphSAGE (E4)       & 0.4344 & 0.9298 & 0.5922 & 0.7143 \\
GS + RF (E3)         & 0.9963 & 1.0000 & 0.9981 & 0.9997 \\
GS + MLP (E3)        & 0.9933 & 1.0000 & 0.9966 & 0.9987 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{full_comparison.png}
\caption{Full comparison across 10 models and four metrics spanning E1 through E5. SVM is the main negative outlier; GraphSAGE (E4) favors recall over precision, while E3 embedding models approach E2 graph-enhanced performance using learned representations.}
...
Key findings:
\begin{enumerate}
    \item \textbf{Graph features improve each supervised model} under the stratified oversampled protocol. LR recall rises from 0.8664 to 0.9740; RF + graph reaches PR-AUC $=$ 1.0000.
    ...
    \item \textbf{SVM underperforms} under our sampling/tuning budget, underscoring that model choice matters.
\end{enumerate}
...
The authors thank Prof.\ Hasan Davulcu and the CSE 573 Semantic Web Mining staff at Arizona State University for guidance. The BankSim dataset was sourced from Kaggle \cite{b1}. Clone and run instructions and links to the GitHub repository appear in Section~\ref{sec:repro}. Figures and metrics reproduce artifacts under \texttt{EVALUATIONS/}; related-work facts are cited and paraphrased by the authors.
...
\bibitem{b5}
D.\ Wang, J.\ Lin, P.\ Cui, Q.\ Jia, Z.\ Wang, Y.\ Fang, Q.\ Yu, J.\ Zhou, S.\ Yang, and Y.\ Qi, ``A semi-supervised graph attentive network for financial fraud detection,'' in \textit{Proc.\ IEEE International Conference on Data Mining (ICDM)}, Beijing, China, 2019, pp.\ 598--607. DOI: 10.1109/ICDM.2019.00070
...
\bibitem{b7}
S.\ Alarfaj, I.\ Malik, H.\ U.\ Khan, N.\ Almusallam, M.\ Ramzan, and M.\ Ahmed, "Credit card fraud detection using state-of-the-art machine learning and deep learning algorithms," IEEE Access, vol. 10, pp. 39700–39715, 2022. DOI: 10.1109/ACCESS.2022.3166891
