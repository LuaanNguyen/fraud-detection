...
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[node distance=0.34cm and 0.3cm]

  \node[io] (csv) {BankSim CSV\\594,643 rows};

  \node[block, below=of csv] (prep)
    {\textbf{Step 1}\\\texttt{CODE/preprocessing.py}\\Clean, encode, scale\\SMOTE-ENN print + time-split tensors};

  \node[block, below=of prep] (eda)
    {\textbf{Step 2}\\\texttt{CODE/eda.py}\\7 EDA plots};

  \node[exp, below=of eda] (e1)
    {\textbf{Step 3}\\\texttt{CODE/models.py}\\E1 baselines};

  \node[exp, below=of e1] (graph)
    {\textbf{Step 4}\\\texttt{CODE/graph.py}\\Neo4j load + graph CSV};

  \node[exp, below=of graph] (hybrid)
    {\textbf{Step 5}\\\texttt{CODE/hybrid\_models.py}\\E2 tabular + graph features};

  \node[exp, below=of hybrid] (e4)
    {\textbf{Step 6}\\\texttt{CODE/graphsage.py}\\E4 end-to-end GNN};

  \node[exp, below=of e4] (e3)
    {\textbf{Step 7}\\\texttt{CODE/embedding\_models.py}\\E3 embeddings + RF/MLP};

  \node[exp, below=of e3] (e5)
    {\textbf{Step 8}\\\texttt{CODE/clustering.py}\\E5 K-Means + DBSCAN};

  \node[out, below=of e5] (out)
...
\caption{Linear view of \texttt{CODE/main.py}: Steps~1--8 run strictly top to bottom. E1/E2 reuse stratified splits after oversampling (Section~\ref{sec:arch}); Step~6 (E4) executes before Step~7 (E3). Artifacts land under \texttt{EVALUATIONS/}; the UI lives under \texttt{CODE/dashboard/}.}
\label{fig:pipeline}
\end{figure}
