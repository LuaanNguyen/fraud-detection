% CSE 573 — submit PDF named: Group23-Project10-S26-Final Project Report.pdf
% Source: compile from repository root so \graphicspath{EVALUATIONS/...} resolves:
%   pdflatex "Group23-Project10-S26-Final Project Report.tex" (run twice)
...
% =========================================================
\begin{abstract}
We built a fraud-detection project on the BankSim synthetic banking dataset (594,643 transactions; fraud about 1.2\%; imbalance $\approx$1:81). Tabular models ignore links between customers and merchants, so we asked whether graph-side signals and a Graph Neural Network help. Our implementation runs eight sequential steps from \texttt{CODE/main.py}: baseline models (E1), hybrid models that add graph-derived features (E2), GraphSAGE embeddings plus downstream classifiers (E3), an end-to-end GraphSAGE classifier (E4), and clustering for fraud rings (E5). Graph features lift every supervised model we compared; Random Forest with graph features reaches PR-AUC $=$ 1.0000. Unsupervised K-Means surfaces a small cluster at about 99.7\% fraud without using labels. We also ship a React~19 dashboard under \texttt{CODE/dashboard/} to browse plots and tables saved under \texttt{EVALUATIONS/}.
\end{abstract}
...
What we deliver matches the public GitHub repo:
\begin{itemize}
    \item One driver script, \texttt{CODE/main.py}, that runs preprocessing, EDA, and experiments E1--E5 in order.
    \item Modules under \texttt{CODE/} for preprocessing, baselines, graph features, hybrids, GraphSAGE, embeddings, and clustering; outputs written to \texttt{EVALUATIONS/}.
...
The runtime entry point is \texttt{CODE/main.py}. Fig.~\ref{fig:pipeline} sketches how raw transactions flow through preprocessing and experiments E1--E5; plots and metrics land under \texttt{EVALUATIONS/}. Per \texttt{README.md}, run commands from the repository root (for example \texttt{python CODE/main.py}). Long hyperparameter searches may require adjusting parallel backend settings on some operating systems.
...
\begin{tikzpicture}[node distance=0.45cm and 0.3cm]
  ...
  % Input
  \node[io] (csv) {BankSim CSV\\594,643 rows};

  % Step 1
  \node[block, below=of csv] (prep)
    {\textbf{Step 1}\\\texttt{CODE/preprocessing.py}\\Clean · Encode · Scale\\SMOTE-ENN demo + time split};

  % Split output
  \node[block, below=of prep] (split)
    {Stratified train/test\\for E1 \& E2\\after oversampling};

  % Step 2 EDA
  \node[block, below left=0.45cm and -0.5cm of split, xshift=-1.2cm] (eda)
    {\textbf{Step 2}\\\texttt{CODE/eda.py}\\7 EDA plots};

  % Three parallel experiment tracks
  \node[exp, below=0.9cm of split, xshift=-2.8cm] (e1)
    {\textbf{Step 3}\\\texttt{CODE/models.py}\\E1};

  \node[exp, below=0.9cm of split] (graph)
    {\textbf{Step 4}\\\texttt{CODE/graph.py}\\Graph build};

  \node[exp, below=0.9cm of split, xshift=2.8cm] (gnn)
    {\textbf{Step 6}\\\texttt{CODE/graphsage.py}\\E4};

  % Step 5 hybrid
  \node[exp, below=0.45cm of graph] (hybrid)
    {\textbf{Step 5}\\\texttt{CODE/hybrid\_models.py}\\E2};

  % Step 7 embedding
  \node[exp, below=0.45cm of gnn] (emb)
    {\textbf{Step 7}\\\texttt{CODE/embedding\_models.py}\\E3};

  % Step 8 clustering
  \node[exp, below=0.45cm of e1] (clust)
    {\textbf{Step 8}\\\texttt{CODE/clustering.py}\\E5};

  % Output
  \node[out, below=1.1cm of hybrid] (out)
    {\texttt{EVALUATIONS/}\\plots \& CSVs\\\texttt{CODE/dashboard/}};

  % Arrows
  \draw[arrow] (csv)   -- (prep);
  \draw[arrow] (prep)  -- (split);
  \draw[arrow] (split) -- (eda);
  \draw[arrow] (split) -- (e1);
  \draw[arrow] (split) -- (graph);
  \draw[arrow] (split) -- (gnn);
  \draw[arrow] (graph) -- (hybrid);
  \draw[arrow] (gnn)   -- (emb);
  \draw[arrow] (e1)    -- (clust);
  \draw[arrow] (e1)    -- (out);
  \draw[arrow] (hybrid)-- (out);
  \draw[arrow] (emb)   -- (out);
  \draw[arrow] (clust) -- (out);

\end{tikzpicture}
\caption{Pipeline aligned with \texttt{CODE/main.py}: preprocessing prints SMOTE-ENN and time-split diagnostics, while E1/E2 train on stratified splits after oversampling (Section~\ref{sec:arch}). Artifacts are saved under \texttt{EVALUATIONS/}; the UI lives under \texttt{CODE/dashboard/}.}
\label{fig:pipeline}
\end{figure}
...
\texttt{CODE/hybrid\_models.py} recomputes degree, PageRank, betweenness, and community features through \texttt{FraudGraph} before oversampling and splitting. Step~4 (\texttt{run\_graph\_pipeline()}) additionally writes consolidated graph-feature exports used elsewhere in the assignment workflow.

\subsection{Baseline Models (E1)}
...
\subsection{GraphSAGE Models (E3, E4)}
...
\subsection{Unsupervised Clustering (E5)}
...
Fig.~\ref{fig:eda_class} shows the severe class imbalance and category-level fraud patterns discovered during EDA. Fig.~\ref{fig:eda_heatmap} shows the feature correlation heatmap, where \texttt{amount} has the strongest single-feature correlation with fraud (0.49).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.49\columnwidth]{01_class_distribution.png}
\hfill
\includegraphics[width=0.49\columnwidth]{02_fraud_by_category.png}
\caption{Left: Class distribution showing 1:81 fraud-to-legitimate imbalance. Right: Fraud rate per merchant category - \texttt{es\_leisure} (87\%) and \texttt{es\_travel} (79\%) are extreme high-risk categories.}
\label{fig:eda_class}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.49\columnwidth]{03_amount_distribution.png}
\hfill
\includegraphics[width=0.49\columnwidth]{07_correlation_heatmap.png}
\caption{Left: Transaction amount distributions - fraudulent transactions skew higher. Right: Feature correlation heatmap - \texttt{amount} (0.49) is the strongest predictor; \texttt{step} shows a negative trend (fraud decreases over time).}
\label{fig:eda_heatmap}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.49\columnwidth]{04_fraud_over_time.png}
\hfill
\includegraphics[width=0.49\columnwidth]{06_merchant_fraud.png}
\caption{Left: Temporal fraud rate trend - fraud decreases over the 180 time steps. Right: Merchant-level fraud rates - 11 of 49 merchants exceed 50\% fraud rate, invisible without graph analysis.}
...
GraphSAGE is trained as an encoder, extracting 32-dimensional node embeddings. Customer and merchant embeddings are concatenated to form a 64-feature transaction vector fed to RF and MLP. GraphSAGE + RF achieves PR-AUC = 0.9997 - nearly matching the manually engineered RF + Graph (1.0000) - without requiring any domain expertise in graph algorithm selection. This validates end-to-end representation learning as a viable substitute for manual graph feature engineering.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.49\columnwidth]{graphsage_loss.png}
\hfill
\includegraphics[width=0.49\columnwidth]{graphsage_pr_curve.png}
\caption{E4: Left - GraphSAGE training loss over 100 epochs showing steady convergence. Right - GraphSAGE PR curve (PR-AUC = 0.7143). The curve shape reflects the high-recall / low-precision tradeoff: the model aggressively identifies fraud at the cost of false positives.}
\label{fig:e4}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{clustering/clustering_results.png}
...
\caption{Full experiment comparison across all 10 models and four metrics. The lift from E1 (tabular baseline) to E2 (graph-enhanced) is visible. SVM is a strong negative outlier. GraphSAGE E4 favors recall over precision, while E3 embedding models approach E2 using learned representations.}
...
The interface has 8 tabs in a sticky header: \textbf{Overview}, \textbf{E1 Baseline}, \textbf{E2 Graph}, \textbf{E3 Embeddings}, \textbf{E4 GNN}, \textbf{E5 Clustering}, \textbf{EDA Insights}, and \textbf{Results Gallery}. Metric values are semantically color-coded: green ($\geq$0.99), amber ($\geq$0.95), yellow ($\geq$0.80), red (below 0.80).
...
The authors thank Prof.\ Hasan Davulcu and the CSE 573 Semantic Web Mining staff at Arizona State University for guidance. The BankSim dataset was sourced from Kaggle \cite{b1}. Clone and run instructions appear in Section~\ref{sec:repro}. Figures and metrics reproduce artifacts under \texttt{EVALUATIONS/}; related-work facts are cited and paraphrased by the authors.
